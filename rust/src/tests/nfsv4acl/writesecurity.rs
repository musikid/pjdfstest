...
fn preserve_sgid(ctx: &mut SerializedTestContext) {
    let path = ctx.new_file(FileType::Regular).mode(0o2755).create().unwrap();
    let user = ctx.get_new_user();
...
fn preserve_suid(ctx: &mut SerializedTestContext) {
    let path = ctx.new_file(FileType::Regular).mode(0o4755).create().unwrap();
    let user = ctx.get_new_user();
...
fn write_acl(ctx: &mut SerializedTestContext) {
    ...
    ctx.as_user(&user, None, || {
        let e = exacl::setfacl(&[&path][..], &entries, AclOption::empty())
            .unwrap_err();
